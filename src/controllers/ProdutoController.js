...
class ProdutoController extends Controller {
  ...
  async pegaTodos(req, res) {
    ...
    try {
      const produtos = await produtoServices.pegaTodosOsRegistros(whereConvertida);
      await mudarIdParaNome(produtos);
  ...
  async pegaUmRegistro(req, res) {
    ...
    try {
      ...
      return res.status(500).json({message: `${error.message} - Erro ao buscar produto`});
    }
  ...
  async buscaFornecedorPorEscopo(req, res) {
    try {
      ...
      const listaProdutos = await produtoServices.pegaFornecedorEscopo(fornecedorId);
      await mudarIdParaNome(listaProdutos);
      return res.status(200).json(listaProdutos);

    } catch (error) {
      res.status(500).json(`${error.message} - Erro ao buscar fornecedor por escopo`);
    }
  }
  ...
  async buscaCategoriaPorEscopo(req, res) {
    ...
    try {
      const listaProdutos = await produtoServices.pegaCategoriaEscopo(categoriaId, subcategoriaId);
      await mudarIdParaNome(listaProdutos);
      return res.status(200).json(listaProdutos);
    } catch (error) {
      return res.status(500).json(`${error.message} - Erro ao buscar por categoria`)
    }
  }
}
...
async function mudarIdParaNome(listaProdutos) {
  try {
    for(const produto of listaProdutos){
      var categoria = await categoriaProdutoServices.pegaUmRegistroPorId(produto.categoria_id);
      produto.categoria_id = categoria.nome;
      var fornecedor = await fornecedorServices.pegaUmRegistroPorId(produto.fornecedor_id);
      produto.fornecedor_id = fornecedor.nome;
      var subcategoria = await subCategoriaProdutoServices.pegaUmRegistroPorId(produto.subcategoria_id);
